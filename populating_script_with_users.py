...
mainUser = User(name = 'Main User', email = 'dummy@mail.com', picture = 'https://redash.io/assets/images/temp/AmazonDynamoDB.png')
...
for cat in categories:
    ...
    if cat == "Soccer":
        for soccerItem in soccer_items:
            item = Items(user_id=1, name=soccerItem, description=soccer_items[soccerItem], category = category_name)
            session.add(item)
            session.commit()
    elif cat == "Basketball":
        for basketballItem in basketball_items:
            item = Items(user_id=1, name=basketballItem, description=basketball_items[basketballItem], category = category_name)
            session.add(item)
            session.commit()
    elif cat == "Baseball":
        for baseballItem in baseball_items:
            item = Items(user_id=1, name=baseballItem, description=baseball_items[baseballItem], category = category_name)
            session.add(item)
            session.commit()
    elif cat == "Snowboarding":
        for snowbItem in snowboarding_items:
            item = Items(user_id=1, name=snowbItem, description=snowboarding_items[snowbItem], category = category_name)
            session.add(item)
            session.commit()
    elif cat == "Hockey":
        for hockeyItem in hockey_items:
            item = Items(user_id=1, name=hockeyItem, description = hockey_items[hockeyItem], category = category_name)
            session.add(item)
            session.commit()
